...
# ========================= data ==========================
train_file = available_corpus["msrvtt_ret_train9k"]
...
model = dict(
    ...
    audio_encoder=dict(
        name='beats',
        d_model=768,
        audio_model_path=f"{_MODEL_PATH}/beats.pth",
    ),
    vision_encoder=dict(
        # backbone
        ...
        pretrained=f'{_MODEL_PATH}/6B_stage2.pth',
        use_checkpoint=True,
        checkpoint_num=48,
        use_flash_attn=flag,
...
# scheduler = dict(sched="cosine", epochs=1, min_lr_multi=0.01, warmup_epochs=1)
