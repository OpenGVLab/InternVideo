...
MODEL_PATH = '/mnt/petrelfs/share_data/likunchang/model/internvideo2'
_MODELS = {
    # set InternVL
    "stage1_1B_pt": os.path.join(MODEL_PATH, "stage1/1B/1B_pt.pth"),
    "stage2_1B_pt": '/mnt/petrelfs/share_data/lixinhao/avp_1b_f4_coco_smit_e4.pt',
    "stage1_6B_pt": os.path.join(MODEL_PATH, "stage1/6B/6B_pt.pth"),
}
...
class InternVideo2(nn.Module):
    def __init__(
        ...
        self.return_index = []
        if clip_return_index:
            self.return_index = clip_return_index
        else:
    ...
    def forward(self, x):
        ...
        for idx, blk in enumerate(self.blocks):
            ...
            if idx in self.return_index:
                if isinstance(x, tuple) and len(x) == 2:
                    ...
                    # if residual is not None: # BUG: Do not return the first one
        ...
        if self.return_attn:
            return z, x, attn[:, 0, 1:] # (B, THW)
        else:
            return z, x


def teacher_internvideo2_1B(
        clip_norm_type='l2',
        return_attn=True,
        clip_return_layer=1,
    ...
    ckpt = torch.load(_MODELS["stage1_1B_pt"], map_location='cpu')['module']
    message = model.load_state_dict(ckpt, strict=False)
    print(message)
    return model


def teacher_internvideo2_stage2_1B(
        clip_norm_type='l2',
        return_attn=True,
        clip_return_layer=1,
        clip_return_interval=1,
    ...
    ckpt = torch.load(_MODELS["stage2_1B_pt"], map_location='cpu')['module']

    new_ckpt = OrderedDict()
    from .pos_embed import interpolate_pos_embed
    interpolate_pos_embed(ckpt, model, orig_t_size=4) # 4 for InternVideo2 stage2
...
def teacher_internvideo2_6B(
    ...
    ckpt = torch.load(_MODELS["stage1_6B_pt"], map_location='cpu')['module']
    message = model.load_state_dict(ckpt, strict=False)
    print(message)
    return model
...
if __name__ == '__main__':
    ...
    model = teacher_internvideo2_1B().cuda().half()
    # model = teacher_internvideo2_6B().cuda().half()
    # print(model)
