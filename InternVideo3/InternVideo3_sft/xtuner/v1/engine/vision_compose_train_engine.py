...
class VisionComposeTrainEngine(TrainEngine):
    ...
    def train_step(self, data_batches: List[ModelItem]) -> tuple[LossLog, OtherLog]:
        ...
        # Use float to avoid overflow: (num_tokens.sum())**2 can exceed 2^63 for long sequences
        efficient_forward_tokens = torch.tensor(0.0, device=DEVICE, dtype=torch.float64)
        total_forward_tokens = torch.tensor(0.0, device=DEVICE, dtype=torch.float64)
        ...
        for i in range(0, len(data_batches), intra_layer_micro_batch):
            ...
            for data in data_batch:
                ...
                num_tokens_f = num_tokens.float()
                efficient_forward_tokens += (num_tokens_f**2).sum()
                total_forward_tokens += (num_tokens_f.sum()) ** 2
