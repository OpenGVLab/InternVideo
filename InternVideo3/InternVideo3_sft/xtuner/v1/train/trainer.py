...
class Trainer:
    ...
    def _log_step(
        ...
        # Clamp to avoid OverflowError: Python int too large to convert to C int (timedelta)
        max_safe_seconds = 2147483647  # max 32-bit signed int
        eta_seconds_clamped = min(max(eta_seconds, 0), max_safe_seconds)
        eta_hms = str(timedelta(seconds=int(eta_seconds_clamped)))
