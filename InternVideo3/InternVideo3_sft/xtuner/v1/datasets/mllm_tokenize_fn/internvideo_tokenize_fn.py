...
from .internvideo_utils import InternVideoOSSLoader, read_internvideo_video
...
def video_smart_resize(
    ...
    factor: int = 28,  # patch_size=14 * merge_size=2
    ...
    # print(f"h_bar: {h_bar}, w_bar: {w_bar}, height: {height}, width: {width}")
    return h_bar, w_bar
...
def replace_video_token(
    ...
    # 对 timestamps_list 进行处理，对当中的每一个数据timestamps, 都进行插值，长度变为原来两倍， 新插入的时间为原来相邻时间戳的平均
    # For each timestamps in timestamps_list, insert between every pair the average, resulting in doubled length
    def interpolate_timestamps(timestamps):
        res = []
        for i in range(len(timestamps) - 1):
            res.append(timestamps[i])
            res.append((timestamps[i] + timestamps[i + 1]) / 2)
        res.append(timestamps[-1])
        res.append(res[-1] + (res[-1] - res[-2]))
        return res

    # if len(timestamps_list) > 0:
    #     for i in range(len(timestamps_list)):
    #         if timestamps_list[i] is not None:
    #             timestamps_list[i] = interpolate_timestamps(timestamps_list[i])
    # breakpoint()
    # print("len(timestamps_list): ", len(timestamps_list[0]), len(num_image_token_list[0]))
...
class InternVideoTokenizeFunction(BaseMLLMTokenizeFunction):
    def __init__(
        self,
        tokenizer: PreTrainedTokenizer,
        processor_path: str,
        anno_name: str,
        min_pixels: int = 4*32*28,  # Max image pixels (H*W) for image
        max_pixels: int = 16384*4*32*28,  # Min image pixels (H*W) for image
        video_min_frames: int = 4,  # Min frames per video
        video_max_frames: int = 768,  # Max frames per video
        ...
        rand_video_max_frames: int = 512,
        fps: int =2,  # Sampling time interval (seconds) between frames
        video_max_total_pixels: int = 20480*4*32*28,  # Max pixels within a frame
        video_min_total_pixels: int = 4*4*32*28,  # Min pixels within a frame
        ...
        if oss_loader_cfg is not None:
            self.oss_loader = InternVideoOSSLoader(
        ...
        _processor = AutoProcessor.from_pretrained(processor_path, trust_remote_code=False)
        # breakpoint()
        ...
        self.video_processor.patch_size = 14
        # breakpoint()
        # self.video_processor.merge_size = 1

        # self.video_processor.temporal_patch_size = 1
        # default min_pixels 4096=4x28x28=4x14x14x2x2 pix 一张图片 patch size=14x14，然后 merge size=2x2, 最终输出给 llm 占 4 个 token
        # default max_pixels 16777216=16384x28x28 pix 一张图片输出给 llm 会占 16384 个 token
        if min_pixels is not None:
            self.image_processor.size["shortest_edge"] = min_pixels
        if max_pixels is not None:
            self.image_processor.size["longest_edge"] = max_pixels
        # default video_min_total_pixels 4096=4x28x28 整个视频输出给 llm 会占 4 个 token
        # default video_max_total_pixels 25165824=20480x28x28 整个视频输出给 llm 会占 20480 个 token
        ...
        self.chat_template = CHAT_TEMPLATE_MAP["internvideo"]
    ...
    def multi_modal_get_item(self, data_item: dict, media_root: str = "") -> QwenVL3DataItem:
        ...
        # print(f"grid_thw: {grid_thw}, image_tensor: {image_tensor.shape}, image_data_list: {len(image_data_list)}, image_data_list[0].size: {image_data_list[0].size}")
        del image_data_list
    ...
    def calc_frame_info(self, data_item):
        ...
        随机采样，并且最终数据不会带任何时间戳。不过每个 video 的 image_wh 在 internvideo 中是必备的
    ...
    def video_get_item(self, data_item: dict, media_root: str = "") -> QwenVL3DataItem:
        ...
        for i, video_path in enumerate(self._video_path):
            ...
            if self.oss_loader is not None:
                ...
                image_list, frame_indices, timestamps = read_internvideo_video(
            ...
            # print(video_data.shape,video_result["pixel_values_videos"].shape)
            # breakpoint()
...
class InternVideoTokenizeFnConfig(BaseMLLMTokenizeFnConfig):
    ...
    def build(
        ...
        return InternVideoTokenizeFunction(
            tokenizer,
            self.processor_path,
            anno_name,
