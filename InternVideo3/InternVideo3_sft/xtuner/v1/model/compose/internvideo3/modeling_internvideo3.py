...
from .internvideo3_config import InternVideo3BaseConfig
...
INTERNVIDEO3_COMPILE_CFG: dict[str, TorchCompileOption] = {
    "xtuner.v1.model.compose.internvideo3.modeling_vision.InternVideo3VisionLayer.forward": TorchCompileOption(fullgraph=True),
    **DEFAULT_FLOAT8_CFG,
}


class InternVideo3ForConditionalGeneration(BaseComposeModel):
    config: InternVideo3BaseConfig

    def __init__(self, config: InternVideo3BaseConfig):
        super().__init__(config)  # type: ignore[arg-type]

        if type(self.language_model) is Qwen3VLTextMoE:
            _hf_prefix = config.language_model_hf_prefix
    ...
    def default_compile_cfg(self) -> dict[str, TorchCompileOption]:
        return INTERNVIDEO3_COMPILE_CFG
