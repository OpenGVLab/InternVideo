...
class ResidualAttentionBlock(nn.Module):
    def __init__(self, d_model, n_head, attn_mask=None, mlp_ratio=4.0):
        super().__init__()
...
class Transformer(nn.Module):
    def __init__(
            ...
            mlp_ratio=4.0, checkpoint_num=0,
        ):
        super().__init__()
        self.layers = layers
        self.resblocks = nn.ModuleList()
        for _ in range(layers):
            self.resblocks.append(ResidualAttentionBlock(width, heads, mlp_ratio=mlp_ratio))
        self.return_index = []
        ...
        if checkpoint_num > 0:
            logger.info(f'Using checkpoint with {checkpoint_num} layers')
...
class VisionTransformer(nn.Module):
    def __init__(
        ...
        clip_return_layer=1, clip_return_interval=1, with_ln_proj=True, mlp_ratio=4.0,
        checkpoint_num=0,
    ):
        super().__init__()
        logger.info(f'Return Layer: {clip_return_layer}')
        logger.info(f'Return Interval: {clip_return_interval}')
        logger.info(f'With LayerNorm and Proj: {with_ln_proj}')
        ...
        self.transformer = Transformer(
            ...
            mlp_ratio=mlp_ratio, checkpoint_num=checkpoint_num,
        )
    ...
    def forward(self, x, mask=None):
        ...
        K = x.size(0)
        x = x.view(K, HW + 1, N, T, C).permute(0, 2, 3, 1, 4).reshape(K, N, T, (HW + 1), C)  # [K, N, THW, C]
        x = self.ln_post(x)
...
def load_state_dict(model, state_dict, input_resolution=224, patch_size=16, center=True):
    ...
    keys_unexpected = [k for k in state_dict.keys() if k not in state_dict_3d.keys()]
    keys_missing = [k for k in state_dict_3d.keys() if k not in state_dict.keys()]
    logger.info(f'Unexpected keys: {keys_unexpected}')
    logger.info(f'Missing keys: {keys_missing}')
...
def clip_vision_G14(
    ...
    model = VisionTransformer(
        input_resolution=input_resolution, patch_size=14, 
        width=1664, layers=48, heads=16, output_dim=1280,
        clip_return_layer=clip_return_layer,
        clip_return_interval=clip_return_interval,
        with_ln_proj=with_ln_proj,
