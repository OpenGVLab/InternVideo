...
from timm.models import load_checkpoint as load_checkpoint_ema
...
def get_best_checkpoint(path_to_job, ema=False):
    """
    ...
    if ema:
        best_path = os.path.join(path_to_job, 'best_ema.pyth')
...
def save_checkpoint(path_to_job, model, model_ema, optimizer, loss_scaler, epoch, cfg):
    """
    Save a checkpoint.
    Args:
    ...
    normalized_ema_sd = sub_to_normal_bn(get_state_dict(model_ema))

    # Record the state.
    checkpoint = {
        "epoch": epoch,
        "model_state": normalized_sd,
        "model_ema": normalized_ema_sd,
...
def save_best_checkpoint(path_to_job, model, model_ema, optimizer, loss_scaler, epoch, cfg, ema=False):
    """
    Save a checkpoint.
    Args:
        model (model): model to save the weight to the checkpoint.
    ...
    # Write the checkpoint.
    model_name = 'best.pyth'
    if ema:
        model_name = 'best_ema.pyth'
    path_to_checkpoint = os.path.join(path_to_job, model_name)
    with g_pathmgr.open(path_to_checkpoint, "wb") as f:
        torch.save(checkpoint, f)
    return path_to_checkpoint
...
def load_test_checkpoint(cfg, model, model_ema):
    """
    ...
    if cfg.TEST.CHECKPOINT_FILE_PATH != "":
        ...
        load_checkpoint_ema(model_ema.module, cfg.TEST.CHECKPOINT_FILE_PATH, use_ema=True)
        ...
        best_ema_checkpoint = get_best_checkpoint(cfg.OUTPUT_DIR, ema=True)
        load_checkpoint_ema(model_ema.module, best_ema_checkpoint, use_ema=True)
    elif has_checkpoint(cfg.OUTPUT_DIR):
        last_checkpoint = get_last_checkpoint(cfg.OUTPUT_DIR)
        load_checkpoint(last_checkpoint, model, None, cfg.NUM_GPUS > 1)
        load_checkpoint_ema(model_ema.module, last_checkpoint, use_ema=True)
        ...
        load_checkpoint_ema(model_ema.module, cfg.TRAIN.CHECKPOINT_FILE_PATH, use_ema=True)
    else:
        logger.info(
...
def load_train_checkpoint(cfg, model, model_ema, optimizer, loss_scaler):
    """
