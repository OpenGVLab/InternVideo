...
def get_path_to_checkpoint(path_to_job, step, save_latest=False):
    """
    Get the full path to a checkpoint file.
    Args:
        path_to_job (string): the path to the folder of the current job.
        step (int): the number of step for the checkpoint.
    """
    if save_latest:
        name = "checkpoint_latest.pyth"
    else:
        name = "checkpoint_step_{:05d}.pyth".format(step)
...
def save_checkpoint(path_to_job, model, optimizer_list, loss_scaler_list, step, cur_step_list, cfg_list, zip=True):
    """
    Save a checkpoint.
    Args:
        model (model): model to save the weight to the checkpoint.
        optimizer_list (optim): optimizer to save the historical state.
        loss_scaler_list (scaler): scaler for loss.
        step (int): current number of step of the model.
        cfg_list (CfgNode): configs to save.
    """
    # Save checkpoints only from the master process.
    if not du.is_master_proc(cfg_list[0].NUM_GPUS * cfg_list[0].NUM_SHARDS):
        return
    # Ensure that the checkpoint dir exists.
    g_pathmgr.mkdirs(get_checkpoint_dir(path_to_job))
    # Omit the DDP wrapper in the multi-gpu setting.
    sd = model.module.state_dict() if cfg_list[0].NUM_GPUS > 1 else model.state_dict()
    ...
    for i in range(len(optimizer_list)):
        checkpoint[f'optimizer_{i}_state'] = optimizer_list[i].state_dict()
        checkpoint[f'scaler_{i}'] = loss_scaler_list[i].state_dict()
        checkpoint[f'cfg_{i}'] = cfg_list[i].dump()
        checkpoint[f'cur_step_{i}'] = cur_step_list[i]
    # Write the checkpoint.
    path_to_checkpoint = get_path_to_checkpoint(path_to_job, step + 1, save_latest=cfg_list[i].TRAIN.SAVE_LATEST)
...
def save_best_checkpoint(path_to_job, model, optimizer_list, loss_scaler_list, step, cur_step_list, cfg_list, zip=True, name='best.pyth'):
    """
    Save a checkpoint.
    Args:
        model (model): model to save the weight to the checkpoint.
...
def load_checkpoint(
    ...
    Args:
        ...
        optimizer_list (optim): optimizer to load the historical state.
    ...
    if convert_from_caffe2:
        ...
        if "step" in checkpoint.keys() and not epoch_reset:
            step = checkpoint["step"]
            if optimizer_list:
                for i, optimizer in enumerate(optimizer_list):
                    optimizer.load_state_dict(checkpoint[f"optimizer_{i}_state"])
            if loss_scaler_list:
                for i, loss_scaler in enumerate(loss_scaler_list):
                    loss_scaler.load_state_dict(checkpoint[f"scaler_{i}"])
            cur_step_list = []
            num = 0
            for k in checkpoint.keys():
                if "cur_step" in k:
                    num += 1
            for i in range(num):
                cur_step_list.append(checkpoint[f"cur_step_{i}"])
        else:
            step = -1
            cur_step_list = None
    return step, cur_step_list
...
def load_train_checkpoint(cfg, model, optimizer_list, loss_scaler_list):
    """
    ...
    if cfg.TRAIN.AUTO_RESUME and has_checkpoint(cfg.OUTPUT_DIR):
        ...
        checkpoint_step, cur_step_list = load_checkpoint(
            last_checkpoint, model, loss_scaler_list, cfg.NUM_GPUS > 1, optimizer_list
        )
        ...
        checkpoint_step, cur_step_list = load_checkpoint(
            best_checkpoint, model, loss_scaler_list, cfg.NUM_GPUS > 1, optimizer_list
        )
        start_step = checkpoint_step + 1
    ...
    return start_step, cur_step_list
