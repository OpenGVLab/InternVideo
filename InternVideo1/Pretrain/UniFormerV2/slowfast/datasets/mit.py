...
class Mit(torch.utils.data.Dataset):
    """
    MiT video loader. Construct the MiT video loader, then sample
    ...
    def __init__(self, cfg, mode, num_retries=10):
        """
        Construct the MiT video loader with a given csv file. The format of
        the csv file is:
        ```
        ...
        assert mode in [
            ...
            "val",
            "test",
        ], "Split '{}' not supported for MiT".format(mode)
        self.mode = mode
        self.cfg = cfg
        ...
        logger.info("Constructing MiT {}...".format(mode))
    ...
    def _construct_loader(self):
        ...
        ), "Failed to load MiT split {} from {}".format(
            self._split_idx, path_to_file
        )
        logger.info(
            "Constructing MiT dataloader (size: {}) from {}".format(
