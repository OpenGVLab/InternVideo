...
class PretrainVisionTransformerEncoder(nn.Module):
    ...
    def forward_features(self, x, mask=None, d_mask=None, use_image=False):
        ...
        if mask is not None:
            if d_mask is not None:
                x_vis = x[~mask].view(B, -1, C) # ~mask means visible
                x_vis = x_vis[~d_mask].reshape(B, -1, C) # ~d_mask means visible
            else:
    ...
    def forward(self, x, mask=None, d_mask=None, use_image=False):
        x_vis, x_clip_vis = self.forward_features(x, mask, d_mask, use_image)
        return x_vis, x_clip_vis
...
class PretrainVisionTransformer(nn.Module):
    ...
    def forward(self, x, mask=None, d_mask=None, use_image=False, keep_temporal=False):
        T = x.shape[2]
        x_vis, x_clip_vis = self.encoder(x, mask, d_mask, use_image) # [B, N_vis, C_e]
        ...
        if mask is not None and len(x_clip_vis) > 0:
            ...
            if d_mask is not None:
                clip_pos_emd_vis = expand_clip_pos_embed[~mask].view(B, -1, C_CLIP)
                clip_pos_emd_vis = clip_pos_emd_vis[~d_mask].view(B, -1, C_CLIP).unsqueeze(0).repeat(K, 1, 1, 1)    
            else:
...
if __name__ == '__main__':
    ...
    # print(time.time()-s)
    mask = torch.cat([
        torch.ones(1, num_frames * int(14 * 14 * 0.75)),
        torch.zeros(1, num_frames * int(14 * 14 * 0.25)),
    ], dim=-1).to(torch.bool)
    print(model(torch.rand(1, 3, num_frames, 224, 224), mask)[1].shape)