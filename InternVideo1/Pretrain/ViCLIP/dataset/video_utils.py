...
def read_frames_list(
        video_path, num_frames, sample='rand', fix_start=None, 
        max_num_frames=-1,  trimmed30=False
    ):
        assert isinstance(video_path, list), "video_path must be a list of frames"
        assert "fps" not in sample
        vlen = len(video_path)
        frame_indices = get_frame_indices(
            num_frames, vlen, sample=sample, fix_start=fix_start,
            input_fps=1, max_num_frames=max_num_frames
        )
        frames = [Image.open(video_path[idx]) for idx in frame_indices]
        frames = torch.stack([torch.from_numpy(np.array(frame)) for frame in frames])  # (T, H, W, C), torch.uint8
        frames = frames.permute(0, 3, 1, 2)  # (T, C, H, W), torch.uint8
