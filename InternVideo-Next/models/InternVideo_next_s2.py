...
class Block(nn.Module):
    ...
    def __init__(
        ...
        self.attn = Attention(
            ...
            qk_normalization=qk_normalization, use_fused_rmsnorm=use_fused_rmsnorm,
            use_rope=use_rope, cls_token_num=cls_token_num)
...
class InternVideo2(nn.Module):
    def __init__(
            ...
            depth: int = 12,
            clip_return_start_depth: int = 12,
        ...
        if clip_student_return_index:
            ...
            for i in range(clip_return_layer):
                self.clip_return_index.append(clip_return_start_depth - int(i * clip_student_return_interval) - 1)
        ...
        self.selfdist_decoder = eval(clip_student_decoder)(
            in_channels=1024,
        ...
        self.selfdist_normproj_decoder = eval(clip_student_decoder)(
            in_channels=embed_dim,
            out_channels=embed_dim,
            norm_layer=partial(nn.LayerNorm, eps=1e-5),
            norm_type=clip_norm_type
        )
        self.selfdist_proj_clip = eval(clip_student_decoder)(
    ...
    def forward_reconstruction_loss(self, x, to_reconstruct, diff_pos_embed, cls_token_num, mask, mask_target, B, T, L, C):
        ...
        return forward_bert_mask_token
    ...
    def forward(self, x, mask=None, mask_target=None, mask_ratio=0., embed_only=False, return_mvd=False):
        cls_token_num = self.cls_token_num
        ...
        if embed_only:
            return x[:, self.cls_token_num:, :], self.clip_projector(x)

        forward_bert_mask_token = self.forward_reconstruction_loss(
            x, to_reconstruct, diff_pos_embed, cls_token_num, mask, mask_target, B, T, L, C)

        forward_bert_mask_token = self.selfdist_decoder(forward_bert_mask_token[:, self.cls_token_num:, :])
        forward_bert_mask_token = forward_bert_mask_token[mask].reshape(B, -1, forward_bert_mask_token.shape[-1])
        forward_bert_mask_token = forward_bert_mask_token / forward_bert_mask_token.norm(dim=-1, keepdim=True)

        if return_mvd:
            x_align = self.selfdist_proj_clip(self.clip_projector(x))
            x_align = x_align / x_align.norm(dim=-1, keepdim=True)

            outputs_vis_middle = self.selfdist_normproj_decoder(x[:, self.cls_token_num:, :])
            outputs_vis_middle = outputs_vis_middle / outputs_vis_middle.norm(dim=-1, keepdim=True)

            return forward_bert_mask_token, outputs_vis_middle, x_align, mask
        else:
            x_align = self.selfdist_proj_clip(self.clip_projector(x))
            x_align = x_align / x_align.norm(dim=-1, keepdim=True)

            return forward_bert_mask_token, x_align


@register_model
def internvideo_next_stage2_base(pretrained=False, **kwargs):
    model = InternVideo2(
...
@register_model
def internvideo_next_stage2_large(pretrained=False, **kwargs):
    model = InternVideo2(
...
@register_model
def internvideo_next_stage2_1b(pretrained=False, **kwargs):
    model = InternVideo2(
