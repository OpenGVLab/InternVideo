...
from optim_factory import (
    ...
    get_parameter_groups_modernbert,
)
from datasets import build_pretraining_dataset, build_multi_pretraining_dataset
from engines.engine_for_stage1 import train_one_epoch
...
def get_args():
    ...
    parser.add_argument('--diffusion_loss_only', action='store_true')
    parser.add_argument('--umt_loss_only', action='store_true')
    parser.add_argument('--wo_middle_loss', action='store_true')

    parser.add_argument('--modernbert_scale', default=1.0, type=float)
    ...
    # Model parameters
    parser.add_argument('--imagenet_standard_mean_and_std', default=False, action='store_true')
    ...
    parser.add_argument('--reconstruction_ratio', default=0.5, type=float,
                        help='ratio of the visual tokens/patches need be masked')
    parser.add_argument('--input_size', default=224, type=int,
                        help='videos input size for backbone')
    parser.add_argument('--drop_path', type=float, default=0.0, metavar='PCT',
                        help='Drop path rate (default: 0.0)')
    parser.add_argument('--normalize_target', default=True, type=bool,
    ...
    parser.add_argument('--cls_token_num', type=int, default=4)
    ...
    parser.add_argument('--clip_loss_ratio', default=[1, 1, 1], type=float, nargs='+', metavar='BETA',
    ...
    parser.add_argument('--clip_teacher_return_index', default=None, type=int, nargs='+', metavar='BETA',
                        help='index of CLIP teacher return layers (default: None)')
    parser.add_argument('--clip_student_return_index', default=None, type=int, nargs='+', metavar='BETA',
                        help='index of CLIP student return layers (default: None)')
    parser.add_argument('--clip_student_decoder', default='MLP_Decoder', type=str,
                        help='type of student decoder')
    ...
    parser.add_argument('--f2', type=int, default=26)
    parser.add_argument('--fs', type=int, default=2)
    parser.add_argument('--r1', type=int, default=168)
    parser.add_argument('--r2', type=int, default=280)
    parser.add_argument('--rs', type=int, default=28)

    parser.add_argument('--base_token_num', type=int, default=2048)
    parser.add_argument('--max_token_limit', type=int, default=4096)
    parser.add_argument('--min_token_limit', type=int, default=2048)
    
    parser.add_argument('--clip_input_frame', default=8, type=int)
...
def get_model(args):
    ...
    model = eval(args.model)(
        ...
        clip_student_return_index=args.clip_student_return_index,
        clip_student_decoder=args.clip_student_decoder,
...
def main(args, ds_init):
    ...
    args.window_size = (args.clip_input_frame // args.tubelet_size, args.clip_input_resolution // patch_size[0], args.clip_input_resolution // patch_size[1])
    ...
    if args.enable_deepspeed:
        loss_scaler = None
        if args.modernbert_scale != 1.0:
            optimizer_params = get_parameter_groups_modernbert(
                model, args.weight_decay, skip_weight_decay_list, modernbert_scale=args.modernbert_scale
            )
        else:
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch(
            ...
            bf16=args.bf16,
            diffusion_loss_only=args.diffusion_loss_only,
            umt_loss_only=args.umt_loss_only,
            wo_middle_loss=args.wo_middle_loss,
        )
        if args.output_dir:
