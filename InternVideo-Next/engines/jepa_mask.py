...
class MaskCollator(object):
    ...
    def __call__(self, batch_size, random_index=0):
        masks_enc = self.mask_generators[random_index](batch_size)
        return masks_enc
...
class _MaskGenerator(object):
    ...
    def __call__(self, batch_size):
        ...
        masks_enc = torch.ones((batch_size, self.duration * self.height * self.width), dtype=torch.int32)
        
        for b in range(batch_size):
            masks_enc[b, collated_masks_enc[b]] = 0
        
        return masks_enc

def get_jepa_mask_collator(
    num_frames: int = 8,
):
    from easydict import EasyDict as edict

    max_keep = int(400 * (num_frames / 8))
    ...
    mymaskcollator = MaskCollator(config, num_frames=num_frames, tubelet_size=1, patch_size=(14, 14))

    return mymaskcollator